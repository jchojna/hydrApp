...
const getSingularOrPlural = (key, value) => {
  const word =
    key === "waterMax" || key === "waterMin" || key === "waterAvg"
...
const getAverageOfArrayValues = (array) => {
  const average =
    array.map((elem) => elem.value).reduce((a, b) => a + b) / array.length
...
const toggleChildrenFocusability = (parent) => {
  ...
  const isException = (button) => {
    return (
      exceptions.filter((exception) => button.classList.contains(exception))
        .length > 0
    )
  }
...
const getSortedUsersIds = () =>
  Object.values(hydrapp.users)
...
const isInputValid = (input, prop, alertBox) => {
  ...
  if (prop === "name") {
    ...
    const isExisting = Object.values(hydrapp.users).find(
...
const updateUsersStats = () => {
  ...
  users.forEach((user) => {
    ...
    const { currentStreak, longestStreak, lastLongestStreakDates, points } =
  ...
  sortedUsersIds.forEach((id, index) => {
    ...
    const prevUser =
      index > 0 ? hydrapp.users[sortedUsersIds[index - 1]] : false
    prevUser
...
const addInitialUsers = () => {
  ...
  initialUsers.forEach((user) => {
    const { name, age, weight, height, dateCreated } = user
...
const getUser = (login) =>
  Object.values(hydrapp.users).find((user) => user.name === login)
...
const setNewUserDOM = () => {
  if (!isNewUserDOM) {
    ...
    // set variables
    const newUserButtons = appNewUser.querySelectorAll(
      "[class*=newUser__button--js]"
    )
    const newUserInputs = appNewUser.querySelectorAll(".newUser__input--js")
...
const handleNewUserQuestion = (index, name) => {
  ...
  currentLabel.textContent =
    index === 0
      ...
      : index === 2
      ? `What\'s your weight, ${name}?`
      : index === 3
      ? `Last question ${name}, what\'s your height?`
      : false
}
...
const handleNewUser = (e) => {
  ...
  if (e.key) {
    ...
    currentQuestion = [...newUserQuestions].find((question) =>
      /visible/.test(question.className)
    )
  ...
  if (slideTimeoutId === null) {
    ...
    const initialOffset =
      action === "prev" ? -1 * containerWidth : containerWidth
    ...
    const shouldQuestionSlide =
      (willGoToNextQuestion &&
        isInputValid(currentInput, currentProp, currentAlert)) ||
      willGoBackToPrevious
    ...
    if (!willGoBackToLogin) {
      if (shouldQuestionSlide) {
        slidePromise
          .then(() =>
            slideQuestions(newIndex, newQuestion, nextInput, finalOffset)
          )
...
const handleWaterMeasure = () => {
  ...
  ;[...measureLevels].forEach((level, index) => {
    ...
    const isLevelOnEnd =
      level.offsetTop < headerHeight + 10 || index === waterMax
...
const handleCounter = (counterObj, currentValue, newValue) => {
  ...
  const { prevTenths, prevOnes, nextTenths, nextOnes } = counterObj
...
const getFeedbackValue = (value) => {
  ...
  return value >= waterMax
    ? 8
    : value >= Math.floor(waterMin + (diff * 3) / 4)
    ? 7
    : value >= Math.floor(waterMin + (diff * 1) / 4)
    ? 6
    : value >= Math.floor(waterMin)
    ? 5
    : value >= Math.floor((waterMin * 4) / 5)
    ? 4
    : value >= Math.floor((waterMin * 3) / 5)
    ? 3
    : value >= Math.floor((waterMin * 2) / 5)
    ? 2
    : value >= Math.floor((waterMin * 1) / 5)
    ? 1
    : 0
}
...
const toggleSidebarTabs = (e) => {
  ...
  if (
    ...
    const currentWeek = [...weeks].find((week) =>
      week.classList.contains("card--visible")
    )
    const rankingCards = rankingContainer.children
    const currentRankingCard = [...rankingCards].find((card) =>
...
const handleCardButtons = (container, index) => {
  ...
  const cardPrevButtons = container.querySelectorAll(
    "[class*=card__button--js-prev]"
  )
  const cardNextButtons = container.querySelectorAll(
    "[class*=card__button--js-next]"
  )
  const areCardsValid =
    cards.length === cardPrevButtons.length &&
...
const handleWeekHeading = () => {
  ...
  const weekHeadings = archiveContainer.querySelectorAll(
    ".card__heading--js-week"
  )
...
const removeLastEntry = (e) => {
  ...
  if (self === removeEntryButton) {
    if (entries.length > 1 && !slideTimeoutId) {
      ...
      loggedUser.entries = [...entries].filter(
        (entry, index) => index !== lastEntryIndex
      )
      ...
      if (day === "sunday") {
        ...
        const weekPrevButton = weekToRemove.querySelector(
          ".card__button--js-prev"
        )
...
const handleEntryEdit = (e) => {
  ...
  const decreaseButton = document.querySelectorAll(
    ".edition__button--js-decrease"
  )[itemIndex]
  const increaseButton = document.querySelectorAll(
    ".edition__button--js-increase"
  )[itemIndex]
...
const handleStats = () => {
  ...
  const loggedUserCard = statsContainer.querySelector(
    `.card--js-${loggedUser.id}`
  )
  const cardIndex = [...loggedUserCard.parentNode.children].indexOf(
    loggedUserCard
  )
...
const handleRankingDOM = () => {
  ...
  const allButtons = rankingContainer.querySelectorAll(
    "[class*=card__button--js"
  )
  if (cardsTotal > 1) {
    ;[...allButtons].forEach((button) =>
      button.addEventListener("click", slideCard)
...
const setSettingsDOM = () => {
  ...
  // add event listeners
  const editButtons = settingsContainer.querySelectorAll(
...
const handleSettingsEdition = (e) => {
  ...
  const settingsCard = settingsContainer.querySelector(".card--js-settings")
  const settingsHeading = settingsCard.querySelector(
    ".card__heading--js-settings"
  )
  ...
  const togglePropDisplay = () => {
    ...
    input.placeholder =
      prop === "weight"
        ...
        ? `${value} cm`
        : prop === "waterMax"
        ? `${getSingularOrPlural("waterMax", value)}`
        : value
  }
  ...
  const exitEditMode = () => {
    ...
    handleContainerHeightThroughTime(
      settingsContainer,
      settingsContainer.firstElementChild,
      transitionTime
    )
  ...
  const handleEdition = (e) => {
    ...
    switch (self) {
      ...
      case saveButton:
        // set new value
        const newValue =
          typeof value === "number" ? parseInt(input.value) : input.value
  ...
  if (typeof value === "number")
    input.addEventListener("keyup", filterUserInput)
}
...
const toggleUserRemoveConfirmation = (e) => {
  ...
  const confirmRemoveButton = userRemove.querySelector(
    ".userRemove__confirm--js"
  )
...
class Entry {
  ...
  set _dateKey(date) {
    this.dateKey = getOffsetedDate(date).toISOString().slice(0, 10)
  }
  get _date() {
    return this.date
  }
...
const landingDay = landingCounterContainer.querySelector(
  ".counter__day--js-landing"
)
const landingDate = landingCounterContainer.querySelector(
  ".counter__date--js-landing"
)
...
const newEntryDay = newEntryCounterContainer.querySelector(
  ".counter__day--js-newEntry"
)
const newEntryDate = newEntryCounterContainer.querySelector(
  ".counter__date--js-newEntry"
)
